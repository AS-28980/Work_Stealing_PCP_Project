...
std::size_t AbpDeque::index_for(std::int64_t logical_index) const {
...
std::string AbpBackend::name() const {
    return "abp";
}

std::size_t AbpBackend::worker_count() const {
    return workers_;
}

EnqueueStatus AbpBackend::enqueue(std::size_t worker_id, Task task) {
...
DequeueResult AbpBackend::dequeue(std::size_t worker_id, std::mt19937_64& rng) {
...
BackendMetrics AbpBackend::metrics() const {
